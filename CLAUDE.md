...
**What this repo is**: Engagement vault for Pawline Veterinary Partners. It is a demo / reference deployment showing Cortex as company-wide KMS. Three-tier visibility per ADR-011: `knowledge-base/` (all-staff, portal-served), `internal/` (exec/board-only, committed, never on the default portal route), `raw/` (local-only, gitignored).
...
- Most-relevant ADRs: ADR-011 (vault visibility tiers; it is the governing visibility ADR), ADR-017 (IP boundary), ADR-021 (umbrella)
